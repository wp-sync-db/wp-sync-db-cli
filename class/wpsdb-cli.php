...
class WPSDB_CLI extends WPSDB_Addon
{
    ...
    public function cli_migration($profile)
    {
        ...
        --$profile;
        if (!$this->meets_version_requirements('1.4b1')) {
            return $this->cli_error(__('Please update WP Sync DB.', 'wp-sync-db-cli'));
        }

        if (!isset($profile)) {
            return $this->cli_error(__('Profile ID missing.', 'wp-sync-db-cli'));
        }

        if (!isset($wpsdb_settings['profiles'][$profile])) {
            return $this->cli_error(__('Profile ID not found.', 'wp-sync-db-cli'));
        }
        ...
        if ('savefile' == $profile['action']) {
            return $this->cli_error(__('Exports not supported for CLI migrations. Please instead select push or pull instead.', 'wp-sync-db-cli'));
        }
        ...
        do {
            foreach ($tables_to_process as $key => $table) {
                ...
                do {
                    ...
                    if (is_wp_error($migrate_table_response = $this->verify_cli_response($response, 'ajax_migrate_table()'))) {
